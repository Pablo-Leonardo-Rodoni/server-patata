...
export const Ul = () => {
  ...
  // Manejo del clic en el botón para abrir/cerrar la lista
  const handleOpen =
    (id: string): React.MouseEventHandler<HTMLButtonElement> =>
    () => {
      console.log(`Button ${id} is open`);
  ...
  return (
    <nav className="side-nav">
      {Object.keys(Data).map((id) => (
        <div className="side-container" key={id}>
          <Button
            eventHandler={handleOpen(id)}
            value={id}
            className="side-btn"
          />
          <svg
          ...
          <div className={`${open[id] ? "active" : ""} side-dropdown-content`}>
            <ul className={`${open[id] ? "active" : ""} side-ul`}>
